...
def get_data(path, n1, n2, n3, n4, col_name='Seq_feature'):
    ...
    test4 = pd.read_excel(path, sheet_name=omics_name[n4]).drop(['Gene_Name', 'label'], axis=1)
    data = data.join(test4)
        ...
        for j in range(i+1, 6):
            for k in range(j + 1, 6):
                for m in range(k + 1, 6):
                    all = get_data(old_Ath, i, j, k, m)
                    new_Ath = pd.merge(all, cita, on='Gene_Name', how='inner')
                    outfile = str(i) + str(j) + str(k) + str(m) + '.csv'
